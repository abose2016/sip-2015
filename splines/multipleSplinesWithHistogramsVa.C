...
vector< vector<double> > cSpline(int nPoints, int npar, vector <double> xData, vector <double> yData, vector <double> yErrorData, double stepSpline, double start, double step)
{
	...
	//Initialize Minuit
	vector<double> vstart, vstep;//-> DONE IN THE MAIN
	for(int i=0; i<npar; i++) 	//set starting values and step sizes for parameters
	{//-> DONE IN THE MAIN
		vstart.push_back(start);//-> DONE IN THE MAIN
		vstep.push_back(step);//-> DONE IN THE MAIN
	}//-> DONE IN THE MAIN
...
void multipleSplinesWithHistogramsVa(int iEventLook = 165 , int nEvents = 1000, int nPoints = 9, double seed = 231) 
{
	...
	//b-spline variables_________________________________________________________________	
	gsl_bspline_workspace *bw = gsl_bspline_alloc(orderSpline, nbreak);
	gsl_vector *xControl = gsl_vector_alloc(nPoints);// -> PASSED AS ARGUMENT
	gsl_vector *yControl = gsl_vector_alloc(nPoints);// -> PASSED AS ARGUMENT 
	gsl_vector *w = gsl_vector_alloc(nPoints);// -> PASSED AS ARGUMENT

	//Create a b spline workspace and allocate its memory
	gsl_bspline_knots_uniform(xminBSplineWorkspace, xmaxBSplineWorkspace, bw);//-> DONE IN THE MAIN
