...
//Data definition
int nControl = 5;
vector <double> xVector, yVector, yErrorVector;
...
double ComputeChi2(vector< double > vpar)
{
	...
	{
		double delta = (yVector.at(i) - vpar.at(i) )/ yErrorVector.at(i);
		chisq += delta*delta;
	}
	return chisq;
}
...
void fcn(int &npar, double *gin, double &f, double *par, int iflag)
{
		...
		//compute the spline
    gsl_spline_init (spline, &xVector[0], &vpar[0], nControl);

    //evaluate the spline
		int nSpline = int ((xVector[nControl-1]-xVector[0])/stepSpline);
    for (int i= 0; i < nSpline; i++)
    {
        xSpline[i] = xVector[0]+i*stepSpline;
...
void tMinuitFit()
{
	...
	FillRandVectors(xVector, yVector, yErrorVector, nControl, seed);
	int nSpline = int ((xVector[nControl-1]-xVector[0])/stepSpline);
