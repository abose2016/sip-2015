...
void multipleSplinesWithHistogramsMerge(int iEventLook = 25, int nEvents = 100, int nPoints = 9, double seed = 231) 
{
	...
	TH1D *hTimeB = new TH1D("Time","; Time [ms]; Number of Events", nbins, xlow, xup); 
	hTimeB->SetStats(0);
	hTimeB->SetLineColor(kBlue);
	hTimeB->SetLineWidth(2);
	hTimeB->SetLineStyle(9);

	TH1D *hTimeC = new TH1D("TimeC","Timing; time [ms]; Number of Events", nbins, xlow, xup); 
	hTimeC->SetLineColor(kRed);
	hTimeC->SetLineWidth(2);
	hTimeC->SetStats(0);
	...
	hInterpB->SetStats(0);
	hInterpB->SetLineColor(kBlue);
	hInterpB->SetLineWidth(2);
	hInterpB->SetLineStyle(9);
	...
	TH1D *hDiffB = new TH1D("Diff B","; Difference: spline - linear interpolation; Number of Events", nbinsd, xlowd, xupd); 
	hDiffB->SetStats(0);
	hDiffB->SetLineColor(kBlue);
	hDiffB->SetLineWidth(2);
	hDiffB->SetLineStyle(9);
	...
	GBspline->SetLineColor(kBlue);
	GBspline->SetLineWidth(2);
	GBspline->SetLineStyle(9);
	Gdata->SetMarkerStyle(20);
	Gdata->SetTitle("");
	Gdata->GetHistogram()->SetMinimum(6);
	Gdata->GetHistogram()->SetMaximum(21);
	Gdata->GetXaxis()->SetTitle("Control points");
	Gdata->GetYaxis()->SetTitle("Arbitrary values");
	...
	//Interpolation 
	TLegend *legInterp = new TLegend(0.6,0.8,0.85,0.9);
	legInterp->SetLineColor(kWhite); 
	legInterp->SetFillColor(kWhite);
	legInterp->SetMargin(0.3); 
	legInterp->SetTextSize(0.04); 
	legInterp->AddEntry(hInterpB,"Basis spline","l");
	legInterp->AddEntry(hInterpC,"Natural cubic spline","l");
	...
	//Test graph
	TLegend *leg = new TLegend(0.15,0.15,0.4,0.35);
	leg->SetLineColor(kWhite); 
	leg->SetFillColor(kWhite);
	leg->SetMargin(0.3); 
	leg->SetTextSize(0.04); 
	leg->AddEntry(Gdata,"Simulated data","p");
	leg->AddEntry(lin, "Linear interpolation", "l");
	leg->AddEntry(GBspline,"Basis spline","l");
	leg->AddEntry(GCspline,"Natural cubic spline","l");
	...
	//Differences
	TLegend *legDiff = new TLegend(0.6,0.8,0.85,0.9);
	legDiff->SetLineColor(kWhite); 
	legDiff->SetFillColor(kWhite);
	legDiff->SetMargin(0.3); 
	legDiff->SetTextSize(0.04); 
	legDiff->AddEntry(hDiffB,"Basis spline","l");
	legDiff->AddEntry(hDiffC,"Natural cubic spline","l");

	hDiffB->SetMaximum(1.1*max(hDiffB->GetMaximum(),hDiffC->GetMaximum()));
	...
	//Time
	TLegend *legTime = new TLegend(0.6,0.8,0.85,0.9);
	legTime->SetLineColor(kWhite); 
	legTime->SetFillColor(kWhite);
	legTime->SetMargin(0.3); 
	legTime->SetTextSize(0.04); 
	legTime->AddEntry(hTimeB,"Basis spline","l");
	legTime->AddEntry(hTimeC,"Natural cubic spline","l");
	...
	std::cout<<"Avg time B: " << hTimeB->GetMean(1) << std::endl;
	std::cout<<"Avg time C: " << hTimeC->GetMean(1) << std::endl;
	std::cout<<"Avg dist B: " << hDiffB->GetMean(1) << std::endl;
	std::cout<<"Avg dist C: " << hDiffC->GetMean(1) << std::endl;
